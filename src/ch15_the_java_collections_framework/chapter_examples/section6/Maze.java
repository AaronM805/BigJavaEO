...
import ch15_the_java_collections_framework.chapter_examples.section6.Path.Direction;

import java.util.ArrayList;

/**
 * A maze has its walls marked by * characters and corridors by spaces. It can classify corridor points as dead ends,
 * intersections, or exits, and it can extend paths from one intersection to another. The maze is assumed not to have any cycles
 * (i.e., paths returning to their own start.)
 */
public class Maze {
    ...
    public ArrayList<Path> pathsFrom(int row, int column) {
        ...
        if (isValid(row - 1, column)) {
            paths.add(new Path(row, column, Direction.NORTH));
        }
        if (isValid(row, column + 1)) {
            paths.add(new Path(row, column, Direction.EAST));
        }
        if (isValid(row + 1, column)) {
            paths.add(new Path(row, column, Direction.SOUTH));
        }
        if (isValid(row, column - 1)) {
            paths.add(new Path(row, column, Direction.WEST));
        }
     ...
     * @param column the column of the position
     *
     * @return the number of neighbors in the four compass directions that are within the maze and not walls.
     */
